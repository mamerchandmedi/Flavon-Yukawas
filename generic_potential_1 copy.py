"""The following code modifies the generic_potential module of cosmoTransitions.
The only modifications are:
 (1) the adaptation of the Coleman-Weinberg and finite temperature potentials to be applicable for the on-shell prescription in which the one-loop contributions donot disturb the minimization conditions at tree-level. See formula (A.2) in our paper. 
 
 (2) add an extra factor of 1/4 in the second term of the energy_density method. To be consistent with eqn (2.7) in our paper."""
...
class generic_potential():
    ...
    def V1T(self, bosons, fermions, T, include_radiation=True):
        ...
        T4 = T*T*T*T
        m2, nb, c, Mphys = bosons
        y = np.sum(nb*Jb(m2/T2), axis=-1)
        m2, nf, Mphys = fermions
    ...
    def Vtot(self, X, T, include_radiation=True):
        ...
        bosons = self.boson_massSq(X,T)
        fermions = self.fermion_massSq(X)
        bosons1 = self.boson_massSq(X,0)
        fermions1 = self.fermion_massSq(X)
        y = self.V0(X)
